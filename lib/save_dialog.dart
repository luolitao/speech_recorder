...
class SaveDialogState extends State<SaveDialog> {
  ...
  Future<Null> initTextControllerWithLargestFileName(
    ...
    print("new $fname");
    String fpath = p.join(directory.path, fname + '.wav');
    setState(() {
  ...
  void _renameAudioFile() async {
    newFilePath =
        p.join(p.dirname(defaultAudioFile.path), _textController.text + '.wav');
  ...
  Future<String> _largestNumberedFilename(
    ...
    try {
      ...
      for (FileSystemEntity entity in entities) {
        ...
        if (filePath.endsWith('.wav') && !(filePath.startsWith('Temp'))) {
